...
func Run(ctx *encoder.RuntimeContext, b []byte, codeSet *encoder.OpcodeSet) ([]byte, error) {
	...
	for {
		...
		case encoder.OpStructHeadIntString:
			...
			// Check OmitZero flag: skip field if value is zero
			u64 := ptrToUint64(p+uintptr(code.Offset), code.NumBitSize)
			v := u64 & ((1 << code.NumBitSize) - 1)
			if code.Flags&encoder.OmitZeroFlags != 0 && v == 0 {
				code = code.NextField
		...
		case encoder.OpStructHeadIntPtr:
			...
			// Check OmitZero flag: skip field if pointer is nil (but include if non-null, even if points to zero)
			if code.Flags&encoder.OmitZeroFlags != 0 && p == 0 {
				// nil pointer with omitzero should be omitted entirely
				code = code.NextField
				break
			}
			b = appendStructKey(ctx, code, b)
		...
		case encoder.OpStructHeadFloat32:
			...
			// Check OmitZero flag: skip field if value is 0.0
			f32Val := ptrToFloat32(p + uintptr(code.Offset))
			if code.Flags&encoder.OmitZeroFlags != 0 && f32Val == 0.0 {
				code = code.NextField
			} else {
				b = appendStructKey(ctx, code, b)
				b = appendFloat32(ctx, b, f32Val)
				b = appendComma(ctx, b)
				code = code.Next
			}
		...
		case encoder.OpStructHeadFloat64:
			...
			// Check OmitZero flag: skip field if value is 0.0
			if code.Flags&encoder.OmitZeroFlags != 0 && v == 0.0 {
				code = code.NextField
			} else {
				b = appendStructKey(ctx, code, b)
				b = appendFloat64(ctx, b, v)
		...
		case encoder.OpStructHeadString:
			...
			// Check OmitZero flag: skip field if value is empty string
			str := ptrToString(p + uintptr(code.Offset))
			if code.Flags&encoder.OmitZeroFlags != 0 && str == "" {
				code = code.NextField
			} else {
				b = appendStructKey(ctx, code, b)
				b = appendString(ctx, b, str)
				b = appendComma(ctx, b)
				code = code.Next
			}
		...
		case encoder.OpStructHeadStringString:
			...
			if code.Flags&encoder.OmitZeroFlags != 0 && str == "" {
				...
				b = appendStructKey(ctx, code, b)
				b = appendString(ctx, b, string(appendString(ctx, []byte{}, str)))
				b = appendComma(ctx, b)
				code = code.Next
			}
		...
		case encoder.OpStructHeadBool:
			...
			// Check OmitZero flag: skip field if value is false
			boolVal := ptrToBool(p + uintptr(code.Offset))
			if code.Flags&encoder.OmitZeroFlags != 0 && !boolVal {
				code = code.NextField
			} else {
				b = appendStructKey(ctx, code, b)
				b = appendBool(ctx, b, boolVal)
				b = appendComma(ctx, b)
				code = code.Next
			}
		...
		case encoder.OpStructHeadArray, encoder.OpStructHeadSlice:
			...
			p += uintptr(code.Offset)
			// Check OmitZero flag: skip field if slice/array is nil
			if code.Flags&encoder.OmitZeroFlags != 0 {
				slice := ptrToSlice(p)
				if slice.Data == nil { // nil slice/array check
					code = code.NextField
					break
				}
			}
		...
		case encoder.OpStructHeadArrayPtr, encoder.OpStructHeadSlicePtr:
			...
			// Check OmitZero flag: skip field if pointer is nil
			if code.Flags&encoder.OmitZeroFlags != 0 && p == 0 {
				code = code.NextField
				break
			}
		...
		case encoder.OpStructHeadMap:
			...
			// Check OmitZero flag: skip field if map is nil
			if code.Flags&encoder.OmitZeroFlags != 0 && p == 0 {
				code = code.NextField
				break
			}
		...
		case encoder.OpStructFieldStringString:
			...
			// Check OmitZero flag: skip field if value is empty string
			if code.Flags&encoder.OmitZeroFlags != 0 && s == "" {
				code = code.Next
				continue
			}
			b = appendStructKey(ctx, code, b)
		...
		case encoder.OpStructFieldSlice:
			p := load(ctxptr, code.Idx)
			p += uintptr(code.Offset)
			// Check OmitZero flag: skip field if slice is nil
			if code.Flags&encoder.OmitZeroFlags != 0 {
				slice := ptrToSlice(p)
				if slice.Data == nil { // nil slice check
					code = code.NextField
					break
				}
			}
		...
		case encoder.OpStructFieldMapPtr:
			...
			// Check OmitZero flag: skip field if map pointer is nil or points to nil map
			if code.Flags&encoder.OmitZeroFlags != 0 && p == 0 {
				code = code.NextField
				break
			}
		...
		case encoder.OpStructEndOmitEmptyNumberPtrString:
			...
			code = code.Next
		case encoder.OpStructEndStruct:
			p := load(ctxptr, code.Idx)
			p += uintptr(code.Offset)
			// Check OmitZero flag: skip field if struct is zero-valued
			if code.Flags&encoder.OmitZeroFlags != 0 {
			...
			b = appendStructEnd(ctx, code, b)
			code = code.Next
		case encoder.OpStructEndOmitEmptyStruct:
			p := load(ctxptr, code.Idx)
			p += uintptr(code.Offset)
			if ptrToPtr(p) == 0 && (code.Flags&encoder.IsNextOpPtrTypeFlags) != 0 {
				b = appendStructEndSkipLast(ctx, code, b)
